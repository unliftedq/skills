...
// Merge manga pages into a PowerPoint .pptx (one page per slide).
// Supports manga input (--cover/--pages) and numbered directory input:
//   node merge-slides.cjs --cover ./cover.png --pages ./manga --output ./one-shot.pptx
//   node merge-slides.cjs ./manga --output ./one-shot.pptx
...
function usage() {
  console.error('Usage:');
  console.error('  node merge-slides.cjs --cover <cover.png> --pages <manga-dir> --output <one-shot.pptx>');
  console.error('  node merge-slides.cjs <numbered-image-dir> [--output <one-shot.pptx>]');
}
...
function collectMangaImages(args) {
  const images = [];
  if (args.cover) {
  ...
  if (args.pages) {
    ...
    const pages = collectNumberedImages(dir);
    assertNoGaps(pages);
    images.push(...pages);
  }
  return images;
...
function collectImages(args) {
  if (args.cover || args.pages) return collectMangaImages(args);
...
function defaultOutput(args, extension) {
  ...
  if (!dir) return path.resolve(`manga.${extension}`);
