...
// Merge manga pages into a single PDF.
// Supports manga input (--cover/--pages) and numbered directory input:
//   node merge-pdf.cjs --cover ./cover.png --pages ./manga --output ./one-shot.pdf
//   node merge-pdf.cjs ./manga --output ./one-shot.pdf
...
function usage() {
  console.error('Usage:');
  console.error('  node merge-pdf.cjs --cover <cover.png> --pages <manga-dir> --output <one-shot.pdf>');
  console.error('  node merge-pdf.cjs <numbered-image-dir> [--output <one-shot.pdf>]');
}
